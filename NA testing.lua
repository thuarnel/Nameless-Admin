...
cmd.add({"ospeed", "orbitspeed"}, {"orbitspeed <number> (ospeed)", "Hat orbit command"}, function(...)
	Speed = tonumber(...)
end)
...
gui.searchCommands = function()
    ...
    for _, v in ipairs(cmdAutofill:GetChildren()) do
        if v:IsA("Frame") and index < 5 then
            ...
            local isMatch = v.Name:find(str) == 1 or (cmd and (cmd[1] == str or cmd[2][1]:lower():find(str) == 1))
            v.Input.Text = str ~= "" and isMatch and (cmd and cmd[1] or v.Name) or name
            v.Visible = str == "" or isMatch
            if v.Visible then
                index = index + 1
...
if IsOnMobile then --basically replicating what Infinite Yield does (add the button only for mobile users)
	...
	TextClickButton.AnchorPoint = Vector2.new(0.5, 0)
